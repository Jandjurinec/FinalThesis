...
def NLCMFO(alpha=1.5, r=3.99, num_of_moths=7, MaxIter =15, max_num_flames=4):
    '''
        Non-linear Levy Chaoitc Moth Flame Optimizer algorithm
    '''
    M = init_population(num_of_moths)
    # M = import_latest_best_params(num_of_moths)
    Convergence_curve = np.zeros(MaxIter)
    Convergence_curve_moth = np.zeros(MaxIter)
    ...
    while t < MaxIter + 1:
        ...
        Convergence_curve_moth[t-1] = M[0, F1_IND]
        ...
        for i in range(num_of_moths):
            for j in range(TRAINABLE_PARAMS):
                ...
                levy_step = levy_flight(a*alpha)
                xchaotic = logistic_map(xchaotic, r)

                # focus bad moths into the flame
                if i <= flame_num:
                    M[i, j] = round(distance * math.exp(levy_step) * math.cos(xchaotic * 2 * math.pi) + best_flames[i, j], PRECISION)
                else:
                    M[i, j] = round(distance * math.exp(levy_step) * math.cos(xchaotic * 2 * math.pi) + best_flames[flame_num, j], PRECISION)
    ...
    return best_flame, Convergence_curve, Convergence_curve_moth
